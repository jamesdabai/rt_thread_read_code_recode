...
static rt_size_t rt_sdcard_read(rt_device_t dev, rt_off_t pos, void* buffer, rt_size_t size)
{
    ...
    //rt_kprintf("cardtype =%d,pos =%08x,size=%d\r\n",CardType,pos ,size);
...
int rt_hw_sdcard_init(void)
{
    ...
    if (SD_Init() == SD_OK)
    {
        ...
        status = SD_ReadBlock(sector,0 , 512);
        if (status == SD_OK)
        {
...
void show_sdcard_info(void)
{
	...
	{
		case SDIO_STD_CAPACITY_SD_CARD_V1_1:rt_kprintf("Card Type:SDSC V1.1\r\n");break;
		case SDIO_STD_CAPACITY_SD_CARD_V2_0:rt_kprintf("Card Type:SDSC V2.0\r\n");break;
		case SDIO_HIGH_CAPACITY_SD_CARD:rt_kprintf("Card Type:SDHC V2.0\r\n");break;
...
void test_SD(void)
{
	...
	while(1)
	{
		...
		{
			//buf=(u8 *)mymalloc(0,512);		//申请内存
			memset((u8 *)buf,0x56,512);
			//if(SD_WriteDisk(buf,520,1)==0)
			{
			    //rt_kprintf("写成功！");
			}
			memset((u8 *)buf,0x00,512);

			if(SD_ReadDisk(buf,8192,1)==0)	//读取0扇区的内容
			{
				//rt_kprintf("SECTOR 0 DATA:\r\n");
				//rt_kprintf("\r\nDATA ENDED\r\n");
